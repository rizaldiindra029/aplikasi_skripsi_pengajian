	...
	public function proses_absen()
	{
		...
		if (!empty($_FILES['bukti']['name'])) {
			...
			if ($this->upload->do_upload('bukti')){
				...
				$this->session->set_flashdata('message', 'swal("Gagal!", "Melakukan absen", "danger");');
				redirect('pegawai');
			}
		
		} 
		else {
			$this->session->set_flashdata('message', 'swal("Kosong!", "Melakukan absen", "danger");');
			redirect('pegawai');
		}
	}
